...
APPLICATION MODE (PWA)
----------------------
Prepared as an installable Progressive Web App (PWA). All project files remain at the repository root. Existing UI and application logic are preserved; only the minimal PWA manifest, mobile app metadata, and service-worker registration were added.

Deployment: connect this repository to Vercel and deploy the root as a static frontend. On a supported mobile browser, use Install/Add to Home Screen to install Learnova.
