...
function totalSpent(){
  return state.paymentHistory.reduce((sum, r) => sum + r.amount, 0);
}

function updateLevelUI(){
  const spent = totalSpent();
  const level = DATA.levelFromSpend(spent);
  const tier = DATA.levelTier(level);
  $all(".level-shield").forEach(el=>{
    el.className = "level-shield lvl-" + tier;
    el.textContent = level;
  });
  const badge = $("#levelShieldBadge");
  if(badge) badge.style.cssText = "position:static;width:52px;height:52px;font-size:20px;border-radius:16px;border-width:2px;";
  const nameEl = $("#levelName");
  if(nameEl){
    const tierLabel = tier === "default" ? "" : " \u00b7 " + tier[0].toUpperCase() + tier.slice(1);
    nameEl.textContent = "Level " + level + tierLabel;
    $("#levelSpend").textContent = formatFcfa(spent) + " spent";
    const nextThreshold = DATA.levelThresholds[Math.min(level+1, 10)];
    const prevThreshold = DATA.levelThresholds[level];
    if(level >= 10){
      $("#levelProgressFill").style.width = "100%";
      $("#levelNextLabel").textContent = "Maximum level reached";
    } else {
      const pct = Math.min(100, Math.round(((spent - prevThreshold) / (nextThreshold - prevThreshold)) * 100));
      $("#levelProgressFill").style.width = pct + "%";
      $("#levelNextLabel").textContent = "Next level at " + formatFcfa(nextThreshold);
    }
    const bonus = DATA.referralBonusForLevel(level);
...
function moveTabIndicator(){
  ...
  clearTimeout(window.__tabIndicatorTimer);
  window.__tabIndicatorTimer = setTimeout(()=>{
    bar.classList.remove("raised");
    indicator.classList.remove("raised");
...
document.addEventListener("DOMContentLoaded", ()=>{
  ...
  window.addEventListener("resize", ()=>{
    ...
    resizeDebounce = setTimeout(moveTabIndicator, 120);
  });
});
...
function initPWAInstall(){
  ...
  if(!prompt || !installBtn || !closeBtn || isStandaloneApp()) return;
  ...
  closeBtn.addEventListener("click", () => {
    prompt.hidden = true;
    sessionStorage.setItem("learnovaInstallDismissed","1");
  });
  if(sessionStorage.getItem("learnovaInstallDismissed") === "1") prompt.hidden = true;
